...
class PDO extends DB {
   ...
   * @param string|DatabaseStatement $value
   * @return bool
   */
  public function isStatement($value) {
    if (is_object($value)) {
      if (strtolower(get_class($value)) == strtolower('DatabaseStatement')) {
   ...
   */
  public function asStatement($value) {
    $o = new DatabaseStatement();
    $o->setType('statement');
    $o->setValue($value);
  ...
  public function setOption($name, $value, $merge = FALSE) {
    if ($name == 'autoParseQueriesEnabled') {
      $this->autoParseQueriesEnabled = $value;
    }
    if ($name == 'tablesInfo') {
      if ($merge && !empty($this->tablesInfo)) {
        $this->tablesInfo = array_merge($this->tablesInfo, $value);
      }
      else {
        $this->tablesInfo = $value;
      }
    }
    ...
    if ($name == 'connectionLink') {
      $this->connectionLink = $value;
    }
    if ($name == 'noQueryExecution') {
      $this->noQueryExecution = $value;
    }
  }

  public function getOption($name) {
    if ($name == 'autoParseQueriesEnabled') {
      return $this->autoParseQueriesEnabled;
    }
    if ($name == 'tablesInfo') {
   ...
   *    C.`company_title`,
   *    (SELECT SUM(`receipts`.`amount`)
   *        FROM `receipts`
   *        WHERE `receipts`.`orderNumber`=O.`id`
   *    ) as 'receipt'
   *    FROM `orders` as O
  ...
  public function getRowsCustom($sqlQuery) {
    $sqlQueryResult = $this->execQueryString($sqlQuery);
  ...
  public function loadCustom($sqlQuery) {
    $sqlResult = $this->execQueryString($sqlQuery);
    if ($sqlResult) {
    ...
    elseif ($row['totalRows'] == $row['maxCode']) {
      ...
      for ($step = 0; $step <= $steps; $step++) {
        ...
        if ($limitStart >= $i) {
          ...
          $__row = $this->loadCustom($sqlQuery);
          //echo " | {$step}-($limitStart to $limitEnd)-{$__row['totalRows']}-".($limitEnd-$limitStart);

          if ($__row['totalRows'] != $limitEnd - $limitStart) {
            $__result = $this->execQueryString("SELECT `$numberColumnName` FROM `$tableName` WHERE `$numberColumnName`>=$limitStart AND `$numberColumnName`<$limitEnd ORDER BY `$numberColumnName` ASC");
            $i = $limitStart - 1;
            while ($__row = $__result->fetch(PDO::FETCH_ASSOC)) {
              $i++;
  ...
  public function getRowNumberCustom($tableName, $uniqueColumnName, $uniqueColumnValue, $sqlOrder = NULL, $sqlWhere = NULL) {
    ...
    */
    $sqlQuery = "SELECT count(*) as 'row_number' FROM `$tableName`
					...
					$sqlWhere
					ORDER BY `$uniqueColumnName` asc " . $sqlOrder;
    $rowNumber = $this->getColumnValueCustom($sqlQuery, 'row_number');
   ...
   * @return string
   */
  public function getGivenLanguageColumnName($tableName, $defaultColumnName, $languageName = 'en', $separator = "|") {
    ...
    if (!empty($defaultColumnName) and !empty($tableName) and !empty($db_name) and !empty($languageName)) {
      ...
      if ($languageName != '' and $languageName != 'en') {
  ...
  */
  ...
  public function isColumnExist($db_name, $tb_name, $co_name) {
    if ($this->isTableExist($db_name, $tb_name)) {
      if (!empty($db_name) and !empty($tb_name)) {
        $columns = $this->listFields($db_name, $tb_name);
   ...
   * @return mixed
   */
  public function numRows(\database\Statement $result) {
    return $result->rowCount();
  }

  /**
   * @param \database\Statement $result
   ...
   * @return mixed
   */
  public function fetchRow(\database\Statement $result) {
   ...
   * @return mixed
   */
  public function fetchAssoc(\database\Statement $result) {
   ...
   * @return int
   */
  public function numFields(\database\Statement $result) {
  ...
  public function affectedRows($result) {
    return $result->rowCount();
    //return $this->prev_stmt->rowCount();
  }
   ...
   * @return array
   */
  public function getTableColumnsName($tableName) {
  ...
  public function registerQuery($sql, $result, $error) {
    if ($this->getOption('debugEnabled') == TRUE) {
      $time = $this->__stopTimer();

      $trace = array();
      $_trace = debug_backtrace();
      foreach ($_trace as &$traceItem) {
        if (!isset($traceItem['file'])) $traceItem['file'] = '';
        if (!isset($traceItem['line'])) $traceItem['line'] = '';
        if (!isset($traceItem['class'])) $traceItem['class'] = '';
        if (!isset($traceItem['function'])) $traceItem['function'] = '';
        $trace[] = $traceItem['file'] . ':' . $traceItem['line'] . ':' . $traceItem['class'] . ':' . $traceItem['function'];
      }
  ...
  public function clearRegisteredQueries() {
    $this->queryStack = array();
  }
   ...
   * @author arash dalir
   */
  public function dumpRegisteredQueries() {
